...
public class PlayerMove : MonoBehaviour
{
  ...
  private void Start()
  {
    ...
    if (GameManager.instance.doEnterMotion)
    {
      this.transform.position = new Vector3(1f, 2f, 0.0f);
      this.EnterStage();
    }
    else
      this.AllowMove();
  ...
  private void Update()
  {
    if (this.allowMove)
    {
      ...
      {
        this.h = Input.GetAxisRaw("HorizontalOne");
        this.v = Input.GetAxisRaw("VerticalOne");
      }
    }
    ...
    if (this.bombNum < GameManager.instance.maxNum && Input.GetButtonDown("Jump") && this.allowMove && this.allowBomb)
    {
      ++GameManager.instance.bombNumInstalled;
      this.InstallBomb();
    }
    this.bombNum = 0;
    if (this.doubleHit >= 2)
      GameManager.instance.heart += this.doubleHit - 1;
    this.doubleHit = 0;
  }
  ...
  private void LateUpdate()
  {
    if (!this.centerCamera)
      return;
    this.mainCamera.transform.position = this.transform.position + Vector3.back * 10f;
    this.blackBoard.transform.position = this.transform.position;
  }
  ...
  private void OnTriggerEnter2D(Collider2D collision)
  {
    ...
    if (!(collision.gameObject.tag == "Item"))
      return;
    ++GameManager.instance.itemNumUsed;
  ...
  private void EnterStage()
  {
    ...
    {
      this.transform.position = new Vector3(1f, this.transform.position.y - 1.2f * Time.deltaTime, 0.0f);
  ...
  private void InstallBomb()
  {
    ...
    if (this.canvasManager.gameObject.name == "CanvasManager")
    {
      ...
      this.bomb[this.installindex - 1].GetComponent<CircularBomb>().bombOwner = 1;
      this.bomb[this.installindex - 1].GetComponent<CircularBomb>().bombPower = (float) GameManager.instance.bombPower;
    ...
    {
      if (!(this.canvasManager.gameObject.name == "ModeManager") || this.canvasManager.GetComponent<ModeManager>().checkBomb[(int) x + 5, -((int) y - 4)])
        return;
      this.bomb[this.installindex++] = Object.Instantiate<Transform>(this.prefab, new Vector3(x, y, 0.0f), Quaternion.identity);
      this.canvasManager.GetComponent<ModeManager>().checkBomb[(int) x + 5, -((int) y - 4)] = true;
      this.bomb[this.installindex - 1].GetComponent<CircularBomb>().canvasManager = this.canvasManager;
      this.bomb[this.installindex - 1].GetComponent<CircularBomb>().bombOwner = 1;
      this.bomb[this.installindex - 1].GetComponent<CircularBomb>().bombPower = (float) GameManager.instance.bombPower;
      this.audioClip[1].Play();
